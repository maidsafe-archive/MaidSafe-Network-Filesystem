...
TEST_F(MessageTest, FUNC_SerialiseThenParse) {
  Message message(action_type_, persona_type_, data_type_, dest_id_, src_id_, content_, signature_);
  ...
  EXPECT_EQ(persona_type_, message2.persona_type());
...
TEST_F(MessageTest, FUNC_SerialiseParseReserialiseReparse) {
  ...
  EXPECT_EQ(persona_type_, message3.persona_type());
